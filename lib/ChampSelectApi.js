...
class ChampSelectApi extends EventEmitter {
    ...
    constructor(replay=false, replay_file="") {
        super()
        ...
        this.api.subscribe("OnJsonApiEvent_lol-champ-select_v1_session", function (data) {
            json_data.jsons.push({"time":Date.now()-start, "data":data})


            if(data.eventType == "Delete"){
                let now = Date.now();
                var fs = require('fs');
                fs.writeFile('logs/log_'+now+ 'json', JSON.stringify(json_data), 'utf8', (err) => {
                    if (err) throw err;
